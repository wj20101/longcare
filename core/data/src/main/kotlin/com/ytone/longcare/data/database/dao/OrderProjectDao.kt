...
interface OrderProjectDao {
    ...
    suspend fun getProjectsByOrderId(orderId: Long): List<OrderProjectEntityDb>
    
    @Query("SELECT * FROM order_projects WHERE order_id = :orderId ORDER BY project_id")
    fun observeProjectsByOrderId(orderId: Long): Flow<List<OrderProjectEntityDb>>
    
    @Query("SELECT * FROM order_projects WHERE order_id = :orderId AND is_selected = 1 ORDER BY project_id")
    suspend fun getSelectedProjects(orderId: Long): List<OrderProjectEntityDb>
    
    @Query("SELECT * FROM order_projects WHERE order_id = :orderId AND is_selected = 1 ORDER BY project_id")
    fun observeSelectedProjects(orderId: Long): Flow<List<OrderProjectEntityDb>>
    ...
    suspend fun insertOrUpdateAll(projects: List<OrderProjectEntityDb>)
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertOrUpdate(project: OrderProjectEntityDb): Long

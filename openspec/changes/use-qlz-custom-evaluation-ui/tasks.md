...
- [x] 3.4 从当前客户/登记业务状态构造上传上下文，可靠字段缺失时传空值且不猜测经纬度；上传成功后继续触发客户详情刷新与 `EVALUATION_COMPLETE`，扩展 ViewModel/导航测试验证报告始终只使用服务端 `pgUrl`。
...
- [x] 4.3 检查 merged manifest、AAR SHA/consumer rules 和 production readiness 守卫，确认没有新增权限、导出组件、`neverForLocation` 声明、secret、Lint/ProGuard 忽略或发布绕过；运行现有 Manifest/厂商 SDK 安全脚本并将已知 production fail-closed 结果如实保留。

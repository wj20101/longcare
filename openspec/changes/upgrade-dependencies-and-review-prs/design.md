...
用户已授权按计划更新/推送现有 PR、为 Compose BOM 新建 PR，并在专项测试与最新 CI 通过后逐项合入；#125 继续暂缓。H5 已提交并推送为 `34d599db`。以下是 GitHub 返回的 head/baseRefOid 快照，所有目标分支均为 `master`；返回的 baseRefOid 是 PR 当时记录值，实际合入前仍以最新主分支重新验证。

| PR | head | API 返回 base | 合并状态 / 检查 |
| --- | --- | --- | --- |
| #115 | 995422dc | fbeaceb5 | CONFLICTING / 无检查 |
| #117 | 01249cb1 | 07173086 | MERGEABLE / 无检查 |
| #120 | 121c948c | 07173086 | MERGEABLE / 旧 verify-build 失败，smoke 跳过 |
| #121 | 20f6811f | 07173086 | MERGEABLE / 旧 verify-build 失败，smoke 跳过 |
| #122 | f02dae26 | 1098bb6b | MERGEABLE / verify-build 成功，smoke 跳过 |
| #123 | dd7defab | 07173086 | MERGEABLE / 旧 verify-build 失败，smoke 跳过 |
| #124 | e1295009 | 07173086 | MERGEABLE / 旧 verify-build 失败，smoke 跳过 |
| #125 | c3a586d3 | 07173086 | MERGEABLE / 暂缓，不更新 Protobuf |
| #126 | da8527cd | 07173086 | MERGEABLE / 旧 verify-build 失败，smoke 跳过 |
| #127 | 74a4d9d7 | 07173086 | MERGEABLE / 旧 verify-build 失败，smoke 跳过 |
| #128 | 234860c1 | 07173086 | MERGEABLE / 旧 verify-build 失败，smoke 跳过 |

#119 于 2026-09-18 关闭且未合入，无需重开。当前目录仍保留 Protobuf 4.28.3、datetime 0.8.0-0.6.x-compat、AGP 9.4.1、Gradle 9.7.1、JDK 21、SDK 24/36/37。再次核对官方发布记录和目标产物：Coil 官网索引有延迟，直接官方 raw changelog 与 Maven POM 确认 3.6.3；Okio 3.18.2 修复旧调用方 Base64 padding 兼容；KSP 2.3.12 最低 AGP 8.12，当前满足且不启用新的 backing field opt-in；Room、Compose BOM、Robolectric、Kotlin 与性能工具目标均保持原计划。

主分支旧运行的重试最初因 CLI 选中只读账号被拒绝；切换到本机已登录的仓库所有者后权限正常。H5 正常推送已触发 [新 CI 35404522867](https://github.com/wj20101/longcare/actions/runs/35404522867)，不再重复启动旧运行，不修改 Lint 内部依赖或配置缓存。

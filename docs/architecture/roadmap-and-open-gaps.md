...
- `app/features` 冻结边界当前是 181 个 Kotlin 文件的精确快照；architecture rule-10 已执行实际文件、allowlist 与本路线图计数的一致性，后续只允许受控收缩，不允许静默扩张或积累陈旧条目。
...
- 统一真机证据工具与 `real_device_acceptance.json` 已经建立：API 36 physical 资格、十类 minified Release smoke、日志脱敏、构建/设备账本、两轮 comparator、报告隐私和两个独立 verdict 均有正反 fixture。当前没有合格 API 36 ARM64 真机与全部账号/订单/NFC/R65C/QLZ BLE/厂商条件，`r8RuntimeAcceptance` 和 `startupProfileBenefit` 都必须继续保持 `unverified`；API 28 真机和 API 37 模拟器不能替代。
...
上述低风险优化批次明确不包含：厂商 SDK 替换或二进制修补、厂商 consumer rules 收窄、Jetifier 关闭、Navigation 迁移、其他 Compose UI 重构，以及 WorkManager、定位、Bugly、DataStore 等启动初始化时序调整。身份、登录与 Home 页面所有权迁移均由独立 OpenSpec change 完成，不改变这些批次的范围。

## P1：`:app` 壳层收敛

`:feature:identification`、`:feature:login` 与 `:feature:home` 已完成各自页面下沉。Home 以唯一 `HomeFeatureScreen` 隔离内部状态、角色 split、护理三页和资源/tests；app 继续持有 Navigation 2、HomeGraph 订单 owner、Sales renderer、Camera/WebView、Startup reporting 和厂商平台适配。三者是后续切片的边界参考。剩余工作按可独立验证的小切片推进：

1. `:feature:photoupload`、`:feature:servicecountdown`
...
正常 Android CI 仍以 compile/lint/assemble 为基础，只在 affected detector 要求时追加 API 36 blocking smoke；Home/Login feature 变更会执行对应 feature compile/unit/lint，并让 app/Home/Login UI tests 进入各自 test APK。CI 仍不自动运行全部项目单元测试、完整用户旅程或厂商真机链路，其余验证由本地 `--full`、专项 workflow、发布验收或真实设备矩阵承担。

统一真机验收的剩余工作是外部执行而不是继续放宽代码门禁：准备一台满足 API 36、physical、`arm64-v8a`、多核和稳定电量/热状态的设备，以及真实测试账号、有效订单、NFC/R65C、相机/定位、腾讯环境、QLZ BLE/运行时 Token、视频对端和更新源；随后用 `run_real_device_acceptance.sh` 完成十类 smoke 与连续两轮 benchmark。只有 `r8RuntimeAcceptance=passed` 时才能关闭 R8 change 任务 5.1，只有 `startupProfileBenefit=verified` 时才能关闭 Profile change 任务 7.5；一个通过不影响另一个，且两者都不解除 production 厂商 blocker。
...
API 37 手动 CI lane 目前用 app、Home feature 与 Login feature 各自的 test APK 覆盖 adaptive window、Home Dashboard/Profile、登录 UI、MessageQueue/反射/native、局域网、Certificate Transparency/网络栈、后台闹钟音频和厂商启动范围；失败会上传 evidence 并保持 blocked，但不阻断 target 36 的正常构建。
...
- 动态 Login/Home 起点、隐私 gate、认证根 `popUpTo`/清栈、重复导航、配置变化、HomeGraph owner、Home 角色分流以及销售返回/保存恢复已建立 focused JVM 与 API 36 等价测试；登录与 Home UI 已迁入 feature，但 route/graph 仍由 app 持有。后续 Login/Home 行为修改或 Nav3 迁移必须复用或等价替换这些契约，并继续覆盖结果返回与消费。
...
- 不把内部功能验证入口升级为普通用户导航。
- 不在 Home 下沉中夹带 Nav3、target/compile SDK、依赖版本、用户存储/Room、WebView host 策略或厂商 AAR/consumer rules 变更。

...
class MainDashboardViewModelTest {
    ...
    private val companyNameProvider = mockk<CompanyNameProvider>(relaxed = true)
    private val unifiedOrderRepository = mockk<OrderDetailRepository>(relaxed = true)
    ...
    fun `loadCompanyName should request latest company name on every entry call`() = runTest {
        coEvery { companyNameProvider.refreshCompanyName() } returnsMany listOf("LongCare", "LongCare Updated")
        ...
        coVerify(exactly = 2) { companyNameProvider.refreshCompanyName() }
        coVerify(exactly = 0) { companyNameProvider.getCompanyName() }
    }

    @Test
    fun `loadCompanyName should keep previous company name when refresh is blank`() = runTest {
        coEvery { companyNameProvider.refreshCompanyName() } returnsMany listOf("LongCare", "")
        val viewModel = createViewModel()
    ...
    @Test
    fun `loadCompanyName should use current-user cached value when initial refresh is blank`() = runTest {
        coEvery { companyNameProvider.refreshCompanyName() } returns null
        coEvery { companyNameProvider.getCompanyName() } returns "LongCare Cached"
        val viewModel = createViewModel()

        viewModel.loadCompanyName()
        advanceUntilIdle()

        assertEquals("LongCare Cached", viewModel.companyName.value)
        coVerify(exactly = 1) { companyNameProvider.refreshCompanyName() }
        coVerify(exactly = 1) { companyNameProvider.getCompanyName() }
    }

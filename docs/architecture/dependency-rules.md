...
- Room schema 变更必须提交 schema JSON 和升级测试，先明确哪些数据需要保留。当前 `DatabaseModule` 缺少迁移路径时会重建全部表，属于存量实现，不可描述为显式 Migration 已落地，也不得借此扩大删库范围。需要保留数据时必须建立对应迁移路径；见系统概览与整体分析的数据章节。

...
interface OrderDao {
    ...
    suspend fun getOrderById(orderId: Long): OrderEntityDb?
    
    @Query("SELECT * FROM orders WHERE order_id = :orderId")
    fun observeOrderById(orderId: Long): Flow<OrderEntityDb?>
    
    @Query("SELECT * FROM orders ORDER BY updated_at DESC")
    fun observeAllOrders(): Flow<List<OrderEntityDb>>
    
    @Query("SELECT * FROM orders WHERE state = :state ORDER BY updated_at DESC")
    fun observeOrdersByState(state: Int): Flow<List<OrderEntityDb>>
    
    // ========== 写入操作 ==========
    
    @Insert(onConflict = OnConflictStrategy.IGNORE)
    suspend fun insertIgnore(order: OrderEntityDb): Long
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertOrUpdateAll(orders: List<OrderEntityDb>)
    
    @Update
    suspend fun update(order: OrderEntityDb)

    @Transaction
    suspend fun insertOrUpdate(order: OrderEntityDb): Long {

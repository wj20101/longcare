...
详细证据、需求矩阵、风险编号与验收方案见[项目整体分析](../analysis/project-review.md)。本文维护可行动的优先级，报告维护判断依据。

- **P1 数据升级边界（R01）**：当前 Room v1/v2 → v3 使用全表重建，与此前文档的显式迁移要求不一致。本次仅修正文档；先确认未上传图片、本地服务状态和文件索引是否允许丢弃，再为下一次 schema 变化制定迁移/重建验收方案。
- **P1 回归调用缺口（R02）**：普通 CI 不跑 App 全量单测，且 `--full` 没有显式运行已存在的 home/location 模块单测。先补充关键链路的执行责任与证据，再讨论扩大门禁。
- **P1 QLZ 异常验收（R03）**：完整主链路已有历史验收记录，但 `use-qlz-custom-evaluation-ui/tasks.md` 的 5.3 仍未完成，不能以主路径成功代替掉线、弱信号、低电量和退出释放等完整硬件矩阵。

## 已接受并延期：厂商安全与兼容问题

Owner 涉及移动端、服务端和厂商；以下是尚未修复的风险治理事项，不再作为本轮已接受风险的发布前置条件：

...
- `R2` 增量切片（`userlist`）：
  - 页面组件拆分：
    - 新增 `UserListComponents.kt`，下沉 `UserListContent` 与 `UserListItem`。
  - 预览拆分：
    - 新增 `UserListPreviews.kt`，统一承接 `UserListItemPreview`、`UserListContentPreview`、`UserListContentEmptyPreview`、`UserListContentLoadingPreview`。
  - 页面职责收敛：

...
check_job_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-ci.yml" "instrumentation-smoke" "Enable KVM acceleration" "sudo[[:space:]]+udevadm[[:space:]]+settle[[:space:]]+--timeout=30" "android-ci waits for KVM udev permissions with a bounded timeout"
...
require_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "name:[[:space:]]*Check vendor SDK risk policy" "android-release keeps the vendor SDK risk check"
...
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "Check vendor SDK risk policy" "verify_vendor_sdk_release_readiness\\.sh" "android-release invokes the vendor SDK risk policy script"
...
python3 "${ROOT_DIR}/scripts/quality/test_release_policy.py"
python3 "${ROOT_DIR}/scripts/quality/test_release_workflow.py"

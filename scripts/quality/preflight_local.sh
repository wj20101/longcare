...
run_local_fast() {
  ...
  if [[ "${MODE}" == "changed-only" ]]; then
    ...
    if has_changed_paths '^(app/src/(main|debug|test)/kotlin/com/ytone/longcare/|integration/|core/|feature/|scripts/quality/(verify_architecture_boundaries\.sh|legacy_feature_files_allowlist\.txt|architecture_legacy_imports_allowlist\.txt|architecture_legacy_import_budget\.txt))'; then
      run_step \
        "architecture-boundaries" \
        bash scripts/quality/verify_architecture_boundaries.sh "${ROOT_DIR}"
    else
    ...
    if has_changed_paths '^(app/src/(main|debug|test)/kotlin/com/ytone/longcare/|integration/|core/|feature/|scripts/quality/verify_module_api_visibility\.sh)'; then
      run_step \
        "module-api-visibility" \
        bash scripts/quality/verify_module_api_visibility.sh app/src/main/kotlin/com/ytone/longcare "${ROOT_DIR}"
    else

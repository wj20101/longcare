...
interface OrderElderInfoDao {
    ...
    suspend fun getByOrderId(orderId: Long): OrderElderInfoEntityDb?
    
    @Query("SELECT * FROM order_elder_info WHERE order_id = :orderId")
    fun observeByOrderId(orderId: Long): Flow<OrderElderInfoEntityDb?>
    
    @Query("SELECT * FROM order_elder_info WHERE elder_user_id = :elderUserId")
    suspend fun getByElderUserId(elderUserId: Int): List<OrderElderInfoEntityDb>
    
    // ========== 写入操作 ==========
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertOrUpdate(elderInfo: OrderElderInfoEntityDb): Long
    
    @Update
    suspend fun update(elderInfo: OrderElderInfoEntityDb)

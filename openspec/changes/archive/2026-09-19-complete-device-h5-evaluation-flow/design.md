...
WebViewActions 默认 JS 关闭与普通返回均只 pop 当前网页。评估入口只标识页面用途，并在其 JS 关闭回调中更新现有销售业务状态，然后 pop 网页；系统返回不标记完成。H5 左上角返回通过 JS 接口关闭，不能与系统返回混称为“顶部返回”。复用现有首页 ViewModel 作用域，不建立返回邮箱或结果关联。普通报告、协议、隐私页面不触发评估完成，也不改变隐私同意。

成功弹窗确认仅刷新网页数据是 H5 当前预期行为，不要求该按钮关闭页面；客户端不监听提交响应、弹窗或 DOM，也不代调用接口。

销售页面观察到业务完成即展示原生完成页。完成页进入时请求结果，H5 不传递等级或业务参数。完成/返回回保留的首页，不自动重开 H5。

### 4. 接口文案

已核对 https://careapi.ytone.cn/swagger/v1/swagger.json：POST /V1/Sale/GetCheckResult，请求 id（客户 ID）、recordId，响应 pgResult、pgUrl（均可空）。虽然文档声明 recordId 可空，2026-09-19 真机确认纯表单省略或传 null 均返回 2001“参数错误”，而最新客户详情有本次等级和报告。按用户确认，有非空设备 recordId 时使用 GetCheckResult；纯表单无设备记录时直接重新请求 GET /V1/Sale/GetUserLatentDetail，映射 pgResult、pgUrl。不读取缓存详情，不在失败后切换接口，不增加兜底链或轮询。
...
- H5 自行控制关闭时机，当前由左上角返回调用；客户端仍按已确认约定处理评估用途的关闭，不再次核实提交状态，不能将关闭接口解释为 H5 提交成功事件。
...
- 评估表单和报告入口显式选择无原生标题栏展示，使用最小展示参数即可；不按 title 文本或 URL 猜测用途，不建立新的容器框架。
- 展示参数与 `isEvaluation` 的业务含义分离：报告无标题栏，但关闭仍只返回来源页，不误触发评估完成。
- H5 填满可用内容区，保留状态栏、导航栏/手势区和显示缺口的安全间距，不要求隐藏系统栏；避免外层 Scaffold 和 WebView 重复加 inset，键盘弹出后表单仍可操作。
- 沉浸式视觉仅在现有 WebViewScreen 中设置白色背景和 AndroidX 浅色系统栏样式，背景铺满全屏，系统栏图标可读；页面暂停/退出时恢复 MainActivity 默认样式。旧系统导航按钮的可读性由 AndroidX 处理，不新增兼容框架、JS、额外业务逻辑或全局主题改造。
- 保留系统返回、加载进度和错误/渲染退出处理。网页异常时即使 H5 返回按钮不可用也能退出；不为异常恢复永久恢复重复标题栏。
- 协议、普通网页和隐私弹窗保持原有展示。更新用途路由、受影响测试和当前文档，先验收本项，再执行独立的依赖/PR 计划。

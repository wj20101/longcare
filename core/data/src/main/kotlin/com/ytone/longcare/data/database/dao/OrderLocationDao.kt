...
interface OrderLocationDao {
    ...
    suspend fun getLocationsByOrderId(orderId: Long): List<OrderLocationEntityDb>
    
    @Query("SELECT * FROM order_locations WHERE order_id = :orderId ORDER BY timestamp DESC")
    fun observeLocationsByOrderId(orderId: Long): Flow<List<OrderLocationEntityDb>>
    
    @Query("SELECT * FROM order_locations WHERE order_id = :orderId ORDER BY timestamp DESC LIMIT 1")
    suspend fun getLatestLocation(orderId: Long): OrderLocationEntityDb?
    
    @Query("SELECT * FROM order_locations WHERE upload_status = :status")
    suspend fun getLocationsByStatus(status: Int): List<OrderLocationEntityDb>

    @Query("SELECT * FROM order_locations WHERE upload_status IN (:statuses) ORDER BY timestamp ASC LIMIT :limit")
    suspend fun getUploadQueue(statuses: List<Int>, limit: Int = 50): List<OrderLocationEntityDb>
    
    // ========== 写入操作 ==========
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(location: OrderLocationEntityDb): Long
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertAll(locations: List<OrderLocationEntityDb>)

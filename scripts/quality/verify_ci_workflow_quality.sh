...
require_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "name:[[:space:]]*Validate QLZ release configuration input" "android-release validates the mode-specific QLZ credential without printing it"
...
require_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "name:[[:space:]]*Verify packaged QLZ SDK artifacts" "android-release verifies QLZ entry classes in packaged APK and AAB"
...
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-ci.yml" "Run ci-required quality gates" "bash scripts/quality/verify_qlz_sdk_artifact\\.sh" "android-ci validates the approved QLZ AAR"
...
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "Run ci-required quality gates" "bash scripts/quality/verify_qlz_sdk_artifact\\.sh" "android-release validates the approved QLZ AAR before release work"
...
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "Validate QLZ release configuration input" "secrets\\.QLZ_PRODUCTION_SDK_KEY" "android-release reads the production QLZ credential from an environment secret"
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "Validate QLZ release configuration input" "secrets\\.QLZ_ACCEPTANCE_SDK_KEY" "android-release reads the acceptance QLZ credential from a separate environment secret"
...
require_pattern "${ROOT_DIR}/app/build.gradle.kts" "dependsOn\\(verifyProductionReleaseConfiguration\\)" "Release assembly and bundle tasks retain the production configuration guard"
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "Build release APK and AAB" "export QLZ_SDK_KEY=" "android-release scopes the selected QLZ credential to the Release build step"
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "Build release APK and AAB" "export QLZ_TEST_MODE=" "android-release scopes the selected QLZ mode to the Release build step"
check_step_contains_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "Verify packaged QLZ SDK artifacts" "verify_qlz_sdk_artifact\\.sh" "android-release packaged-artifact step invokes the QLZ checker"
require_absent_pattern "${ROOT_DIR}/.github/workflows/android-release.yml" "echo[[:space:]]+.*QLZ_SDK_KEY=" "android-release never echoes the selected QLZ credential"
